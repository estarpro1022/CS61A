...
def tail_points(opponent_score):
    ...
    return mul(2, abs(second_rightmost - rightmost)) + 1
    # END PROBLEM 2
...
def perfect_square(score):
    """Check score can be squared by a number.
    ...
    digit = 1
    while digit <= score:
        if pow(digit, 2) == score:
            return True
        digit += 1
    return False

def next_perfect_square(score):
    assert perfect_square(score), 'score equals d * d, where d is an integer.'
    # sqrt returns a float number, e.g. sqrt(1) equals 1.0
...
def is_always_roll(strategy, goal=GOAL):
    ...
    while score0 < goal:
        # remember to initialize score1 with zero each turn
        score1 = 0
        while score1 < goal:
            cur_dice = strategy(score0, score1)
            if cur_dice != first_dice:
                return False
            score1 += 1
        score0 += 1
...
def max_scoring_num_rolls(dice=six_sided, total_samples=1000):
    ...
    while num_dice <= 10:
        ...
        score_averaged_dice = averaged_dice(num_dice, dice)
        if highest < score_averaged_dice:
            highest = score_averaged_dice
            highest_index = num_dice
        num_dice += 1
...
def final_strategy(score, opponent_score, threshold=12, num_rolls=6):
    """Write a brief description of your final strategy.

    choose 0 dice if score would increase by at least threshold points
    if score is greater equal than 80, choose 2 dice.
    """
    # BEGIN PROBLEM 12
    return 0
